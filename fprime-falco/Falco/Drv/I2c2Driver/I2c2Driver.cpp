#include <Falco/Drv/I2c2Driver/I2c2Driver.hpp>
#include <FpConfig.hpp>
#include "Fw/Types/Assert.hpp"
#include "twihs/master/plib_twihs2_master.h"
#include "FreeRTOS.h"
...
namespace Falco {
  ...
  I2c2Driver::I2c2Driver(const char *const compName) : I2c2DriverComponentBase(compName) {
    eventGroupHandleI2c2 = xEventGroupCreate();
    TWIHS2_CallbackRegister(I2c2Callback, reinterpret_cast<uintptr_t>(this));
  }

  I2c2Driver::~I2c2Driver() {

  }

  void I2c2Driver::TWIHS2_Deinitialize() {
  ...
  void I2c2Driver::i2cBusRecovery() {
    // 1. Disable TWIHS
    TWIHS2_Deinitialize();
  ...
  void I2c2Driver::configureGPIOPins() {
    ((pio_registers_t*)PIO_PORT_D)->PIO_PDR = 0x807c8000;
    ((pio_registers_t*)PIO_PORT_D)->PIO_PER = 0x7f837fff;
    ((pio_registers_t*)PIO_PORT_D)->PIO_MDER = 0x18000000;
    ((pio_registers_t*)PIO_PORT_D)->PIO_MDDR = 0xe7ffffff;
    ((pio_registers_t*)PIO_PORT_D)->PIO_OER = 0x1a825000;
    ((pio_registers_t*)PIO_PORT_D)->PIO_ODR = 0xe57dafff;
  }

  void I2c2Driver::configureI2CPins() {
    ((pio_registers_t*)PIO_PORT_D)->PIO_PDR = 0x987c8000;
    ((pio_registers_t*)PIO_PORT_D)->PIO_PER = 0x67837fff;
    ((pio_registers_t*)PIO_PORT_D)->PIO_MDER = 0x0;
    ((pio_registers_t*)PIO_PORT_D)->PIO_MDDR = 0xffffffff;
    ((pio_registers_t*)PIO_PORT_D)->PIO_OER = 0x1a825000;
    ((pio_registers_t*)PIO_PORT_D)->PIO_ODR = 0xe57dafff;
  }

  void I2c2Driver::I2c2Callback(uintptr_t thisObjects) {
    reinterpret_cast<I2c2Driver *>(thisObjects)->callback();
  }

  void I2c2Driver::callback() {
    EventBits_t status = static_cast<EventBits_t>(TWIHS2_ErrorGet());
    BaseType_t xHigherPriorityTaskWoken{0};
    if (status == TWIHS_ERROR::TWIHS_ERROR_NONE) {
      status = transactionComplete;
    }
    xEventGroupSetBitsFromISR(eventGroupHandleI2c2, status, &xHigherPriorityTaskWoken);
  }

  bool I2c2Driver::waitTransactionEnd(U32 timeoutMs) {
    auto ticks = timeoutMs / portTICK_PERIOD_MS; 
    auto transactionResult = xEventGroupWaitBits(eventGroupHandleI2c2,
  ...
  //! Read data
  Drv::I2C::Status I2c2Driver::readData(U32 addr, Fw::Buffer& fwBuffer, U32 timeout)
  {
    auto i2cStatus = Drv::I2C::Status::SUCCESS;

    bool readStatus = TWIHS2_Read(addr, fwBuffer.getData(), fwBuffer.getSize());
    ...
    if (readStatus == true) {
      ...
      auto errorReason = TWIHS2_ErrorGet();
  ...
  //! Write data
  Drv::I2C::Status I2c2Driver::writeData(U32 addr, Fw::Buffer& fwBuffer, U32 timeout)
  {
    auto i2cStatus = Drv::I2C::Status::SUCCESS;

    bool writeStatus = TWIHS2_Write(addr, fwBuffer.getData(), fwBuffer.getSize());
    ...
    )
  {
    ...
    bool writeReadStatus = TWIHS2_WriteRead(addr, writeBuffer.getData(), writeBuffer.getSize(), readBuffer.getData(), readBuffer.getSize());
