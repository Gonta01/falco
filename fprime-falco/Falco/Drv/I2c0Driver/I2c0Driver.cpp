#include <Falco/Drv/I2c0Driver/I2c0Driver.hpp>
#include <FpConfig.hpp>
#include "Fw/Types/Assert.hpp"
#include "twihs/master/plib_twihs0_master.h"
#include "FreeRTOS.h"
...
namespace Falco {
  ...
  I2c0Driver::I2c0Driver(const char *const compName) : I2c0DriverComponentBase(compName) {
    eventGroupHandleI2c0 = xEventGroupCreate();
    TWIHS0_CallbackRegister(I2c0Callback, reinterpret_cast<uintptr_t>(this));
  }
  ...
  void I2c0Driver::TWIHS0_Deinitialize() {
  ...
  void I2c0Driver::i2cBusRecovery() {
    // 1. Disable TWIHS
    TWIHS0_Deinitialize();
  ...
  void I2c0Driver::configureGPIOPins() {
    ((pio_registers_t*)PIO_PORT_A)->PIO_PDR = 0xdfdffe63;
    ((pio_registers_t*)PIO_PORT_A)->PIO_PER = 0x2020019c;
    ((pio_registers_t*)PIO_PORT_A)->PIO_MDER = 0x18;
    ((pio_registers_t*)PIO_PORT_A)->PIO_MDDR = 0xffffffe7;
    ((pio_registers_t*)PIO_PORT_A)->PIO_OER = 0x200018;
    ((pio_registers_t*)PIO_PORT_A)->PIO_ODR = 0xffdfffe7;
  }

  void I2c0Driver::configureI2CPins() {
    ((pio_registers_t*)PIO_PORT_A)->PIO_PDR = 0xdfdffe7b;
    ((pio_registers_t*)PIO_PORT_A)->PIO_PER = 0x20200184;
    ((pio_registers_t*)PIO_PORT_A)->PIO_MDER = 0x0;
    ((pio_registers_t*)PIO_PORT_A)->PIO_MDDR = 0xffffffff;
    ((pio_registers_t*)PIO_PORT_A)->PIO_OER = 0x200018;
    ((pio_registers_t*)PIO_PORT_A)->PIO_ODR = 0xffdfffe7;
  }

  void I2c0Driver::I2c0Callback(uintptr_t thisObjects) {
    reinterpret_cast<I2c0Driver *>(thisObjects)->callback();
  }

  void I2c0Driver::callback() {
    EventBits_t status = static_cast<EventBits_t>(TWIHS0_ErrorGet());
    BaseType_t xHigherPriorityTaskWoken{0};
    if (status == TWIHS_ERROR::TWIHS_ERROR_NONE) {
      status = transactionComplete;
    }
    xEventGroupSetBitsFromISR(eventGroupHandleI2c0, status, &xHigherPriorityTaskWoken);
  }

  bool I2c0Driver::waitTransactionEnd(U32 timeoutMs) {
    auto ticks = timeoutMs / portTICK_PERIOD_MS; 
    auto transactionResult = xEventGroupWaitBits(eventGroupHandleI2c0,
  ...
  //! Read data
  Drv::I2C::Status I2c0Driver::readData(U32 addr, Fw::Buffer& fwBuffer, U32 timeout)
  {
    auto i2cStatus = Drv::I2C::Status::SUCCESS;

    bool readStatus = TWIHS0_Read(addr, fwBuffer.getData(), fwBuffer.getSize());
    ...
    if (readStatus == true) {
      ...
      auto errorReason = TWIHS0_ErrorGet();
  ...
  //! Write data
  Drv::I2C::Status I2c0Driver::writeData(U32 addr, Fw::Buffer& fwBuffer, U32 timeout)
  {
    auto i2cStatus = Drv::I2C::Status::SUCCESS;

    bool writeStatus = TWIHS0_Write(addr, fwBuffer.getData(), fwBuffer.getSize());
    ...
    )
  {
    ...
    bool writeReadStatus = TWIHS0_WriteRead(addr, writeBuffer.getData(), writeBuffer.getSize(), readBuffer.getData(), readBuffer.getSize());
