...
namespace DebugLogger {
    ...
    bool LogFile::setLogFile(const char* fileName, const U32 maxSize, const U32 maxBackups)
    {
        ...
        NATIVE_INT_TYPE stat = snprintf(fileNameFinal,Fw::String::STRING_SIZE, "%s%s", fileName, fileFormat);
        ...
        while (Os::FileSystem::getFileSize(fileNameFinal,tmp) == Os::FileSystem::OP_OK) {
            ...
            NATIVE_INT_TYPE stat = snprintf(fileNameFinal,Fw::String::STRING_SIZE,
                                            "%s%"PRIu32 "%s", fileName, suffix, fileFormat);
        ...
        Os::File::Status fileStatus = this->m_file.open(fileNameFinal, Os::File::OPEN_CREATE, false);
